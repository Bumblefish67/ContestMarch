 ...
 */
class PlayState8 extends FlxState
{
	...
	private var bosstmr:Float = 2;
	private var fBall:Fireball;
	private var fBalls:FlxTypedGroup<Fireball>;
	...
	override public function create():Void
	{
		...
		cointxt.size = 16;
		bg = new FlxSprite(0, 0, "assets/Map1.png");
		add(bg);
		FlxG.worldBounds.setSize(768, 432);
		player = new Player(100, 300);
		boss = new Boss2(600, 200);
		add(zomblood = new FlxTypedGroup<Blood>()); 
		add(Player.bar);
		add(player);
		add(Boss2.bar);
		add(boss);
		add(fBalls = new FlxTypedGroup<Fireball>()); 
   ...
   function startp6(Sprite:FlxSprite):Void
	...
	function bhitboss():Void
	{
		...
		function shootfire():Void
		{
			fBall = new Fireball(boss.x, boss.y);
			fBalls.add(fBall);
		}
		...
		override public function update(elapsed:Float):Void
	{
		...
		for (fBall in fBalls)
		{
		if (fBall.isOnScreen() == false)
		{
			fBalls.remove(fBall);
		}
		if (FlxG.overlap(player,fBall))
		{
			fBalls.remove(fBall);
			player.health -= 5;
		}
		FlxVelocity.moveTowardsObject(fBall, player,300,0 );		
	}
	bosstmr -= FlxG.elapsed;
	if (bosstmr <= 0)
	{
	bosstmr = 2;
	shootfire();
